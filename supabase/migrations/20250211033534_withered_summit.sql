/*
  # Fix vessels table field types

  1. Changes
    - Change created_at to timestamptz
    - Change updated_at to timestamptz
    - Add missing date and entered_by columns
*/
...
-- Create vessels table with correct field types
CREATE TABLE vessels (
    ...
    CONSTRAINT vessels_date_entered_by_check CHECK (
        (date IS NULL AND entered_by IS NULL) OR
        (date IS NOT NULL AND entered_by IS NOT NULL)
    )
);
